...
public class ControladorIO {
	...
	public static void escribeFicheroObjetos(ArrayList<Empleado> listado) throws IOException {
		//PROGRAMANDO BIEN NO HACE FALTA CUAJAR EL CÓDIGO CON TRY-CATCH
		if(!fichero.exists()) {
		...
		for (Empleado e : listado) {
			oos.writeObject(e); //ESCRIBO EL OBJETO DE LA LISTA PASADA POR PARÁMETROS EN EL FICHERO
		}
		...
		if(!fichero.exists()) {
			...
			try{
				while(true) { //LEYENDO EL FICHERO
					Empleado empleado = (Empleado) ois.readObject(); //LEO UN OBJETO GENÉRICO (Clase Object) Y LO CASTEO A MI TIPO PARTICULAR DE OBJETO
